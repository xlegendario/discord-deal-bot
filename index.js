...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isButton() && interaction.customId === 'confirm_deal') {
    ...
    const existingUnits = await base('Inventory Units')
      .select({ filterByFormula: `{Ticket Number} = "${orderNumber}"`, maxRecords: 1 })
      .firstPage();
    ...
    if (!imageMsg) return interaction.reply({ content: '❌ No image found.', flags: 0 });

    const dealMsg = messages.find(m => m.embeds.length > 0);
    const embed = dealMsg?.embeds?.[0];
    if (!embed || !embed.description) return interaction.reply({ content: '❌ Missing embed.', flags: 0 });
    ...
    if (!sellerRecords.length) {
      return interaction.reply({ content: '❌ Seller not found.', flags: 0 });
    }
    ...
    await interaction.reply({ content: '✅ Deal confirmed and added.', flags: 0 });
  }
});
